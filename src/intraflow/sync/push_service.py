...
from intraflow.services.errors import RevisionConflictError, SyncError
...
class PushService:
    ...
    def push_project(self, project_id: str) -> int:
        ...
        self._record_success("PROJECT", project_id, snapshot.revision, payload, clear_outbox=False)
        return snapshot.revision

    def push_pending(self, *, current_user_id: str) -> int:
        with self.session_factory() as session:
            targets = list(session.scalars(
                session.query(SyncOutbox).filter_by(target_type="USER_PUBLIC", target_id=current_user_id).statement
            ))
        completed = 0
